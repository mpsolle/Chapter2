...
public class VacuumEnvironment {
     ...
     VacuumEnvAgent agent = null;
      ...
      /** 
          ...
          * The internal squares have a 10% chance of being dirty. 
          *
      **/
    ...
    public VacuumEnvironment(double dirtyProb, VacuumAgent initAgent, int row, int col){       
     ...
     /** Initializes a rectangular floor pattern with given width and height
      *  and places an agent in the environment a the given location.  If the 
      *  location is an obstacle the agent is placed at the next open location in
      *  row-major order.
    ...
    public VacuumEnvironment(int width, int height, double dirtyProb, VacuumAgent initAgent, int x, int y){
     ...
     /**
      * Creates a floor pattern using geometry and places an agent in the environment at the given
      * location.  If the location is an obstacle the agent is placed at the next open location in
      *  row-major order.
      * @param geometry contains geometry of the floor.  
      *                 - a clean square, D a dirty square, any other character is a border
      * @param initAgent  agent designed to clean floor
      * @param x  x-coor (col) of the location in which to place agent     
      * @param y  y-coor (row) of the location in which to place agent 
      */
         ...
         for (int gRow=0; gRow<geometry.length; gRow++){
             for (int gCol=0; gCol<geometry[gRow].length(); gCol++){
                 switch (geometry[gRow].charAt(gCol)){
                     case 'D' :
                     case 'd' : floor[gRow+1][gCol+1]=Status.DIRTY; break;
                     case ' ' :
                     case '-' : floor[gRow+1][gCol+1]=Status.CLEAN; break;
                     default  : floor[gRow+1][gCol+1]=Status.OBSTACLE; break;                     
                 }
             }
     ...
     private void computeSquareCount(){     
         initSqCount=0; initDirtSqCount=0;
         for (int r=0; r<maxHeight; r++)
             for (int c=0; c<maxWidth; c++ ){
      ...
      * @return if geometryOnly is true, the floor is return as an completely clean floor. Each cell of the 2D array 
      *         contains either the Status.OBSTACLE character or the Status.CLEAN character
      *         Otherwise, the character representation of the floor is returned.
      */
      ...
      * @param col  
      * @param row 
      * @return the status of the floor at location  col, row.  
      *         If col or row is outside the floor dimensions, getStatus
      *         returns Status.OBSTACLE.
      */
     ...
     /**
       ...
       * @param row
       * @param col
       * @return an array of boolean indicating whether or not a given action will cause
       *         agent to enter a cell containing an obstacle.
       */ 
     ...
     /** 
      * Simulates the vacuum agent until one of the following occurs:
      *      1.)  The agent returns a STOP action
      *      2.)  maxTimeSteps have completed
      *      3.)  The performance measure has reached the performTarget
      * 
     ...
     public int simulate(int maxTimeSteps, double performTarget, PerceptType ptype){
         ...
         if(maxTimeSteps <=0) maxTimeSteps = 4*targetTime;
    ...
    public double performance(int currentTime, int maxTimeSteps){
     ...
     class VacuumEnvAgent {
         ...
         /**
          * 
          * @param action performs the appropriate action on the environment
          *               and updates the performance statistics
          */
