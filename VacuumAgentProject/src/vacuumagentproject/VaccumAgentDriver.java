...
public class VaccumAgentDriver {
   ...
   /**
     * an enum that packages all command line options together. User can use -h option to print 
     ...
     * @return  the intCount integer parameters for the command line option in position pos in args.
     */
    static int[] checkInts(String[] args, int pos, int intCount){
        ...
        else System.err.printf("Error:  Missing arguments for option %s\n", args[pos]); 
        return result;
    }
    /**
     * 
     ...
     * @return    the doubleCount double parameters for the command line option in position pos in args.
     */
    ...
    /**
     * 
     * @param geometryFile  a file that contains the floor geometry to be used in simulation
     * @throws IOException 
     */
        ...
        while ((argPos < args.length) && !parseError){
            if (args[argPos].charAt(0) == '-'){
                switch (OPTIONS.valueOf(args[argPos].substring(1))){
                    case h : usage(); break;
                    case d : intResult = checkInts(args, argPos, 2);
                             if (intResult == null)
                                 parseError = true;
                             else{
                                 maxWidth = intResult[0]; maxHeight = intResult[1];   
                             }  
                             argPos +=2;
                             break;
                   case a : intResult = checkInts(args, argPos, 2);
                             if (intResult == null)
                                 parseError = true;
                             else{
                                 agentX = intResult[0]; agentY = intResult[1];   
                             }         
                             argPos +=2;
                   ...
                   case A :  try{
                             ...
                             argPos++;
                             break;    
                   case t :  intResult = checkInts(args, argPos, 1);
                             if (intResult == null)
                                 parseError = true;
                             else {
                             ...
                             argPos++;
                             break;      
                   case p :  doubleResult = checkDoubles(args, argPos, 1);
                   ...
                   case P :  doubleResult = checkDoubles(args, argPos, 1);
                             ...
                             argPos++;
                             break;    
                   case v : useVisualizer = true; break;
                   case B : usePercept = VacuumEnvironment.PerceptType.BUMP; break;
                   case L : usePercept = VacuumEnvironment.PerceptType.LOCATION; break;
                   case g : if ((argPos+1) < args.length){
                               try {                                  
